...
/** OAuth identity providers whose supplied email is already verified at the source. */
const TRUSTED_EMAIL_PROVIDERS = new Set(["google"]);

/**
 * True when the account was created via a trusted OAuth provider (e.g. Google)
 * that hands back an already-verified email — so no in-app OTP step is needed.
 * Accepts the Supabase auth user (or anything carrying app_metadata).
 */
export function isProviderVerifiedEmail(user: {
  app_metadata?: { provider?: string | null; providers?: string[] | null } | null;
} | null | undefined): boolean {
  const meta = user?.app_metadata;
  if (!meta) return false;
  if (meta.provider && TRUSTED_EMAIL_PROVIDERS.has(meta.provider)) return true;
  return (meta.providers ?? []).some((p) => TRUSTED_EMAIL_PROVIDERS.has(p));
}

/**
 * Mark a profile's email as verified because it originated from a trusted OAuth
 * provider. Idempotent; the profile row must already exist. Safe to skip silently
 * on failure since it only affects the email_verified flag.
 */
export async function markEmailVerifiedFromProvider(
  userId: string,
  ...
  const normalized = rawEmail ? normalizeEmail(rawEmail) : null;
  ...
  const { error } = await (admin.from("profiles") as any)
    ...
    .eq("id", userId);
  if (error) {
    console.error("[EMAIL_OTP] Failed to mark provider email verified:", error.message);
  }
}
