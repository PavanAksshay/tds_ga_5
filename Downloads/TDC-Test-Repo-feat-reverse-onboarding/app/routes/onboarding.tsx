...
const BASE_STEP_ORDER: StepId[] = [
  "welcome", "role", "identity", "avatar", "org", "links", "experience", "tech", "extras", "email", "phone", "done",
];
  ...
  return (
    <div className={styles.stepContent}>
      ...
      <p className={styles.privacyNote}>
        We will only use it for community communication purpose only, We won't sell it!
      </p>
      <PhoneVerify
        initialVerified={initialVerified}
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  const { getEmailStatus, isProviderVerifiedEmail } = await import("../services/email-verification.server");
  const { getPhoneStatus } = await import("../services/phone-verification.server");
  const headers = new Headers();
  const user = await getSessionUser(request, headers);
  
  console.log("[ONBOARDING] Onboarding loader check:", { 
    isLoggedIn: !!user, 
    email: user?.email,
  ...
  if (!user) {
    console.warn("[ONBOARDING] No session in onboarding loader, redirecting to login.");
    return redirect("/login", { headers });
  }
  
  const [gate, emailStatus, phoneStatus] = await Promise.all([
    getProfileAuthGateStatus(user.id),
    getEmailStatus(user.id),
    getPhoneStatus(user.id),
  ]);

  // Google (and other trusted OAuth) sign-ins arrive with an already-verified
  // email, so the in-app OTP step is skipped for them. Manual email sign-ups
  // still go through verification.
  const emailProviderVerified = isProviderVerifiedEmail(user);
  ...
  return Response.json({
    ...
    emailVerified: emailStatus.email_verified || emailProviderVerified,
    emailProviderVerified,
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const { isProviderVerifiedEmail, markEmailVerifiedFromProvider } = await import("../services/email-verification.server");
  ...
  // Trusted OAuth sign-ins (e.g. Google) skip the email OTP step, so persist the
  // verified flag here now that the profile row is guaranteed to exist.
  if (isProviderVerifiedEmail(user)) {
    await markEmailVerifiedFromProvider(user.id, user.email);
  }
...
export default function OnboardingPage() {
  const { userId, hasGithub, githubHandle, emailVerified, emailProviderVerified, emailAddress, phoneVerified, phoneNumber } = useLoaderData<typeof loader>();
  const actionData = useActionData<{ error?: string }>();

  // Google (trusted OAuth) accounts arrive pre-verified, so drop the email step
  // entirely. Manual email sign-ups keep it.
  const STEP_ORDER = useMemo<StepId[]>(
    () => (emailProviderVerified ? BASE_STEP_ORDER.filter((s) => s !== "email") : BASE_STEP_ORDER),
    [emailProviderVerified]
  );
  ...
  }, [stepIndex, data, transition, isEmailVerified, isPhoneVerified, STEP_ORDER]);
