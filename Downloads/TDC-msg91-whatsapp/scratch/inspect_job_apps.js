...
async function inspectJobApps() {
  console.log('Checking career_applications structure...');
  const { data: row, error } = await supabase.from('career_applications').select('*').limit(1);
  if (error) {
    console.error(error);
    return;
  }
  console.log('Keys in career_applications row:', Object.keys(row[0] || {}));
}
